...
依赖应用，需在 `sentinel-dashboard` 启动前启动:
...
-Dsentinel.zookeeper.address=localhost:2181 \
-jar target/sentinel-dashboard.jar
```

上述命令中我们指定几个 JVM 参数，其中 `-Dserver.port=8081` 是 Spring Boot 的参数，
用于指定 Spring Boot 服务端启动端口为 `8081`。其余几个是 Sentinel 客户端的参数。
...
| 参数 | 作用 |
|--------|--------|
|`-Dcsp.sentinel.dashboard.server=localhost:8081`|向 Sentinel 接入端指定控制台的地址|
|`-Dproject.name=sentinel-dashboard`|向 Sentinel 指定应用名称，比如上面对应的应用名称就为 `sentinel-dashboard`|

本 dashboard 的限流规则 provider 和 publisher 均已定制化为 zookeeper，使用时需添加以下JVM参数:

| 参数 | 作用 |
|--------|--------|
|`-Dsentinel.zookeeper.address=localhost:2181`|指定 zookeeper 地址|

其他的配置项可以参考 [启动配置项文档](https://github.com/alibaba/Sentinel/wiki/%E5%90%AF%E5%8A%A8%E9%85%8D%E7%BD%AE%E9%A1%B9)。

经过上述配置，控制台启动后会自动向自己发送心跳。程序启动后浏览器访问 `localhost:8081` 即可访问 Sentinel 控制台。
