...
public class SentinelApiClient {
    ...
    private boolean isCommandNotFound(int statusCode, String body) {
        return statusCode == 400 && StringUtil.isNotEmpty(body) && body.contains(
                CommandConstants.MSG_UNKNOWN_COMMAND_PREFIX);
    }
        ...
        try {
            return executeCommand(ip, port, FETCH_GATEWAY_FLOW_RULE_PATH, false)
                    .thenApply(r -> {
                        ...
                        List<GatewayFlowRuleEntity> entities = gatewayFlowRules.stream().map(
                                rule -> GatewayFlowRuleEntity.fromGatewayFlowRule(app, ip, port, rule)).collect(
                                Collectors.toList());
                        return entities;
                    });
        } catch (Exception ex) {
